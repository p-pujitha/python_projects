...
def test_function_call_with_keyword_arguments():
    ...
    assert [10,20,3] == demo(10,second=20) #TypeError: demo() takes atleast 1 argument
...
def simple_format(format, *args):
    ...
    """
    Returns a formatted string by replacing all instances of %X with Xth argument in args
     (0...len(args))
    ...
    while(x<len):
        if(format[x] == '%'):
            ...
            if(k>=temp):
                break
            temp2 = '%'+str(k)
            format = format.replace(temp2,args[k])
            len = format.__len__()
        x=x+1
    return format
