...
def get_word_with_least_vowels(list1):
    list1=list(list1)
    len1=len(list1)
    result=[]
    ovels=['a','e','i','o','u']
    ...
    for x in range(len1):
        ...
        for y in range(len2):
            if innlist[y] in ovels:
                count+=1
        result.append(count)
    return list1[result.index(min(result))]
...
def get_min_max_words(input):
   ...
   # pass # replace this calls to min and max
    ...
    for x in range(len(input)):
       a.append(len(list(input[x])))
    result=(input[a.index(min(a))],input[a.index(max(a))])
...
-the concepts of lists and tuples
-functions
