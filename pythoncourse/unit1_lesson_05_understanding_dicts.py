...
def test_dictionary_is_mutable():
    ...
    assert {"three" : 3, "two" : 2, "one" : 1} == word_to_digit

    del word_to_digit["one"]
    assert {"three" : 3, "two" : 2} == word_to_digit

    word_to_digit["one"] = 10
    assert {"three" : 3, "two" : 2, "one" : 10} == word_to_digit
...
def test_valid_dictionary_keys():
    ...
    try:
        ...
        print te  #observe the error message. prints "UNHASHABLE TYPE : 'LIST' "
        assert True
    ...
    try:
        ...
        print te
        assert True #do we reach here? Yes ->prints unhashable type : 'list'
