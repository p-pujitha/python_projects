...
Python builtins like sequences (strings, lists, tuples), sets and dicts are iterable (ie)
 you can call iter(obj) on them
...
def test_int_iterable():
    try:
        ...
        print te  # it prints 'int' object is not iterable
...
def test_join():
    ...
    try:
        ...
        print te   # it prints sequence item 0: expected string, int found
        assert True
...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [] == result  #what happened here? -> all the items in the iter1 are already iterable
                        # so there is not element in iter1
...
def test_find_builtins_that_work_on_iterables():
    funcs = [all, any, filter, frozenset,iter,list,map,max,min,next,reduce,sorted,sum,tuple,zip]
    assert 15 == len(funcs)
...
- iterators -> iter(object)
- we call the next element using next() method
