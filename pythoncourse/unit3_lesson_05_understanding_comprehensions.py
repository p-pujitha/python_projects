...
def test_dc_mapping():
    ...
    assert 5 == len(result)
    assert {'A':1, 'C':3, 'B':2, 'E':5, 'D':4}== result

def test_dc_nested():
    result = { (x,y): x+y for x in range(2) for y in range(2)}
    assert 4 == len(result)
    assert {(0,1):1, (1,0):1, (0,0):0, (1,1):2}== result
...
def test_sc_filtering():
    ...
    assert set([0,8,2,4,6]) == evens

    odds = {x for x in all if x%2 == 1}
    assert set([1,3,5,7,9]) == odds
...
def get_scrabble_scorer():
    lettar=[]
    num=[]
    for y,x in scrabble_scores:
        for i in range(len(x)):
            lettar.append(x[i])
            num.append(y)


    return {lettar[i]:num[i] for i in range(len(lettar))}
