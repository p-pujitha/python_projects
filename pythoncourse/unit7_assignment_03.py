...
def profile_profiler():
    ...
    for i in range(5):
       (cProfile.run(unit7_conversion_methods.numbers_string1(count)))

    for i in range(5):
        (cProfile.run(unit7_conversion_methods.numbers_string2(count)))


    for i in range(5):
        (cProfile.run(unit7_conversion_methods.numbers_string3(count)))

    for i in range(5):
        (cProfile.run(unit7_conversion_methods.num_strings4(count)))
