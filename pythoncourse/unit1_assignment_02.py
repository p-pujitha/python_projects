...
2. Start thinking in terms of converting one datatype to another based on what you need. Think
   in terms of set and list operations where required instead of thinking in terms of for loops
   and while loops.
...
def get_lower_to_upper_dict():
    l1=list(set(string.ascii_lowercase))
    l2=list(set(string.ascii_uppercase))
    """
...
def get_encoding_dict():
    ...
    l3=list(range(1,27,1))
    a=dict(zip(l2,l3)).copy()
    a.update(dict(zip(l1,l3)))
    """
...
def capitalize_names(input):
    ...
    """
    return  map(lambda str:str.capitalize(),input)
